...
export const Route = createFileRoute("/_authenticated/calendar")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Revisit your Digital Diary entries from a calm monthly calendar.",
      },
  ...
  return (
    <AppShell>
      <div className="page-in">
        ...
        <div className="grid gap-8 lg:grid-cols-[minmax(0,28rem)_1fr]">
          <section className="paper-sheet p-4 sm:p-6">
            <Calendar
              ...
              modifiers={{
                written: Array.from(entryDates).map((key) => new Date(`${key}T12:00:00`)),
              }}
              modifiersClassNames={{
                written:
          ...
          <section>
            ...
            {selectedKey && entryDates.has(selectedKey) ? (
              <Link
                ...
                className="mt-4 block paper-sheet p-5 hover:shadow-lift"
              >
              ...
              <div className="mt-4 space-y-3">
                {(data?.entries ?? []).map((entry) => (
                  <Link
                    ...
                    className="block border-b border-border py-3 hover:text-primary"
                  >
                    ...
                    <span className="ml-3 font-serif text-lg">
                      {entry.mood ? `${entry.mood} mood` : "A written day"}
                    </span>
                  </Link>
                ))}
                {!data?.entries.length && (
                  <p className="font-sans text-sm text-muted-foreground">
                    No pages written this month.
                  </p>
                )}
              </div>
            )}
        ...
        <div className="mt-10 flex items-center gap-2 font-sans text-sm text-muted-foreground">
          <CalendarDays className="h-4 w-4" /> Underlined days have a page.
        </div>
      </div>
    </AppShell>
  );
